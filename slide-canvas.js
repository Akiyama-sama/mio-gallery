const photobox = {
	...
	init() {
		...
		this.total_width =
			this.row_max * (this.img_width + this.img_margin) - this.img_margin;
		this.total_height =
		...
		this.handle_drag_move = this.handle_drag_move.bind(this);
	...
	resize() {
		...
		let line_width = document.querySelector(
		...
		if (body_width > body_height) {
			...
			this.scale_num = 1 - (2 * line_width) / body_height;
		}
	},
	// 创建图片数据即img_data
	create_img_data() {
	...
	create_events() {
		...
		// Mouse Events
		this.canvas.addEventListener("mousedown", this.handle_drag_start);
		this.canvas.addEventListener("mousemove", this.handle_drag_move);
		this.canvas.addEventListener("mouseup", this.handle_drag_end);
		this.canvas.addEventListener("mouseleave", this.handle_drag_end);

		// Touch Events
		this.canvas.addEventListener("touchstart", this.handle_drag_start, {
			passive: false,
		});
		this.canvas.addEventListener("touchmove", this.handle_drag_move, {
			passive: false,
		});
		this.canvas.addEventListener("touchend", this.handle_drag_end);
		this.canvas.addEventListener("touchcancel", this.handle_drag_end);
	},
	...
	handle_drag_move(e) {
		...
		if (e.type === "mousemove") {
			...
			movementX = touch.clientX - this.lastTouchX;
			movementY = touch.clientY - this.lastTouchY;
			// 更新上一次的触摸位置
		...
		this.velocityX = movementX;
		this.velocityY = movementY;
	},
	...
	animate_inertia() {
		...
		this.animationFrameId = requestAnimationFrame(
			this.animate_inertia.bind(this),
		);
	},
	// 移动所有图片
	move_imgs(x, y) {
	...
	check_img(x, y) {
		let img = this.img_data.find(
			(img) =>
				x >= img.x &&
				x < img.x + this.img_width &&
				y >= img.y &&
				y < img.y + this.img_height,
		);
		if (img) {
